...
plot(pu_allyrs$price ~ pu_allyrs$year, ylab = "price", xlab = "year", pch=19, col=COL[1])
dev.off()
...
par(mar=c(4,4.5,1,1), las=1, mgp=c(3,0.7,0), cex.lab = 1.5, cex.axis = 1)
plot(pu$price ~ pu$year, ylab = "price", xlab = "year", pch=19, col=COL[1])
dev.off()
...
plot(m1$residuals ~ pu$year, ylab = "residuals", pch=19, col=COL[1], ylim = c(-10000,10000))
abline(h = 0, lty = 3)
dev.off()
...
par(mar=c(4,4.5,1,1), las=1, mgp=c(3,0.7,0), cex.lab = 1.5, cex.axis = 1)
plot(log(pu$price) ~ pu$year, ylab = "log(price)", xlab = "year", pch=19, col=COL[1])
m2 = lm(log(pu$price) ~ pu$year)
abline(m2, col = COL[4], lwd = 3)
dev.off()
...
par(mar=c(2,4.5,1,1), las=1, mgp=c(3,0.7,0), cex.lab = 1.5, cex.axis = 1)
plot(m2$residuals ~ pu$year, ylab = "residuals", pch=19, col=COL[1], ylim = c(-1.5,1.5))
abline(h = 0, lty = 3)
dev.off()
