...
par(mfrow=c(2,2))
plot(m_full$residuals ~ m_full$fitted.values, col = COL[1,3], pch = 19, main = "Residuals vs. Fitted")
abline(h = 0, lty = 3)
hist(m_full$residuals, col = COL[1], main = "Histogram of residuals")
qqnorm(m_full$residuals, col = COL[1,3], pch = 19, main = "Normal Prob. Plot of residuals")
qqline(m_full$residuals, col = COL[1])
plot(m_full$residuals, col = COL[1,3], pch = 19, main = "Residuals vs. Order of data collection")
abline(h = 0, lty = 3)
dev.off()

## log
m_full_log = lm(log(income) ~ hrs_work + race + age + gender + citizen, data = acs_sub)

pdf("diag_log.pdf", width = 10, height = 7)
par(mfrow=c(2,2))
plot(m_full_log$residuals ~ m_full_log$fitted.values, col = COL[1,3], pch = 19, main = "Residuals vs. Fitted")
abline(h = 0, lty = 3)
hist(m_full_log$residuals, col = COL[1], main = "Histogram of residuals")
qqnorm(m_full_log$residuals, col = COL[1,3], pch = 19, main = "Normal Prob. Plot of residuals")
qqline(m_full_log$residuals, col = COL[1])
plot(m_full_log$residuals, col = COL[1,3], pch = 19, main = "Residuals vs. Order of data collection")
abline(h = 0, lty = 3)
dev.off()
