...
download("https://stat.duke.edu/courses/Spring15/sta101.001/post/labs/sta101_lab4.Rmd", destfile = "sta101_lab4.Rmd")
```
</div>
...
* Note that normal probability plot C has a slight stepwise pattern. Why do you think this is the case?
...
par(mfrow=c(4,2))
# standardize
s_bii.di = (fdims$bii.di - mean(fdims$bii.di))/sd(fdims$bii.di)
s_elb.di = (fdims$elb.di - mean(fdims$elb.di))/sd(fdims$elb.di)
s_age = (bdims$age - mean(bdims$age))/sd(bdims$age)
s_che.de = (fdims$che.de - mean(fdims$che.de))/sd(fdims$che.de)
# bii.di
hist(s_bii.di, main = "Histogram of female bii.di", col = COL[1], xlab = "")
# Plot A
qqnorm(s_che.de, col = COL[1,3], main = "Plot A", pch = 19)
qqline(s_che.de, col = COL[1])
# elb.di
hist(s_elb.di, main = "Histogram of female elb.di", col = COL[1], xlab = "")
# Plot B
qqnorm(s_bii.di, col = COL[1,3], main = "Plot B", pch = 19)
qqline(s_bii.di, col = COL[1])
# age
hist(s_age, main = "Histogram of age", col = COL[1], xlab = "")
# Plot C
qqnorm(s_elb.di, col = COL[1,3], main = "Plot C", pch = 19)
qqline(s_elb.di, col = COL[1])
# che.de
hist(s_che.de, main = "Histogram of female che.de", col = COL[1], xlab = "")
# Plot D
qqnorm(s_age, col = COL[1,3], main = "Plot D", pch = 19)
qqline(s_age, col = COL[1])
```
