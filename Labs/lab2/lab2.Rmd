...
download("http://stat.duke.edu/courses/Spring15/sta101.001/labs/sta101_lab2.Rmd", destfile = "sta101_lab2.Rmd")
```
</div>
...
Suppose you want to recode the `exerany` variable to use informative level names, instead of 0/1. Remember that this variable indicates whether the respondent exercised in the past month (1) or did not (0). We'll create a new variable called `exerany2` and fill it in with `exercised` and `didn't exercise` where we have 1s and 0s, respectively, for the original variable.

```{r eval=FALSE}
cdc$exerany2 = NA  # create new variable, and fill it in with NAs first
cdc$exerany2[cdc$exerany == 1] = "exercised"
cdc$exerany2[cdc$exerany == 0] = "didn't exercise"
cdc$exerany2 = as.factor(cdc$exerany2)  # save new variable as factor
```
...
```{r eval=FALSE}
cdc$genhlth2 = NA  # create new variable, and fill it in with NAs first
cdc$genhlth2[cdc$genhlth %in% c("fair", "poor")] = "bad"
cdc$genhlth2[cdc$genhlth %in% c("good", "very good", "excellent")] = "better"
cdc$genhlth2 = as.factor(cdc$genhlth2)  # save new variable as factor
```
...
Some steps with the survey data...
