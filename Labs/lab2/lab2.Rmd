...
download("https://stat.duke.edu/courses/Spring15/sta101.001/post/labs/sta101_lab2.Rmd", destfile = "sta101_lab2.Rmd")
```
</div>
...
This returns the names `genhlth`, `exerany`, `hlthplan`, `smoke100`, `height`, `weight`, `wtdesire`, `age`, and `gender`. Each one of these variables corresponds to a question that was asked in the survey. A list of the variables and what they represent is given below:

* `genhlth`: general health - excellent, very good, good, fair or poor
* `exerany`: whether the respondent exercised in the past month (1) or did not (0)
* `hlthplan`: whether the respondent had some form of health coverage (1) or did not (0)
...
Sometimes you might need to alter the dataset you have in some way to make it better serve your analysis goals. In this section you will learn a few useful "data wrangling" skills. These might come in especially handy when you're working on your projects.

### Changing level names

Suppose you want to recode the `exerany` variable to use informative level names, instead of 0/1. Remember that this variable indicates whether the respondent exercised in the past month (1) or did not (0). We'll create a new variable called `exerany_cat` and fill it in with `exercised` and `didn't exercise` where we have 1s and 0s, respectively, for the original variable.
...
5.  Create a box plot of BMI vs. this new variable and confirm that the level names have been changed.

### Re-leveling data

You might also want to combine certain levels and re-level a variable. Suppose in the `genhlth` variable, you want to combine the levels `fair` and `poor` to indicate `bad` health, and `good`, `very good`, and `excellent` to indicate `better` health.
...
6.  Make a plot of this new variable vs. another variable and confirm that the change has been implemented.

We can also re-level numerical variables to be categorical. Suppose we want to create a categorical variable for height, `height_cat`, that identifies people's heights to be `below median` and `at or above median`.
...
Let's make a table of this new variable to view its distribution:
```{r eval=FALSE}
table(cdc$height_cat)
```

The output lists the `at or above median` level first, and `below median` level second. This is because by default R lists levels of a categorical variable in alphabetical order. To change the order, we need to specify the order we would like the levels to be displayed:
```{r eval=FALSE}
cdc$height_cat = factor(cdc$height_cat, levels = c("below median", "at or above median"))
```

7.  Confirm that the order of the levels has been changed by making another table of the `height_cat` variable.
...
In this section you will work with data from the class survey you designed. Instead of downloading the data directly into RStudio via a URL (like we did at the beginning of this and the last lab), you will first download the data file to your computer and then upload it to RStudio. We wanted to show you this alternative approach of loading data since this is likely what you will be using for your project.

Click [here](https://stat.duke.edu/~mc301/data/surveyS15.html) to view the codebook for the class survey data. You can use the following to download the data:

```{r eval=FALSE}
download("https://stat.duke.edu/~mc301/data/surveyS15.csv", destfile = "surveyS15.csv") 
```

The data are released in CSV (comma separated values) format, which is a commonly used format for storing tabular data (spreadsheet) in plain-text form.

Next, you will need to import the data into your workspace:

```{r eval=FALSE}
surveyS15 = read.csv("surveyS15.csv")
```

Note that you should do this in your Console as well as in your Markdown document.

Answer the following questions using the survey data. You are welcomed to use the variables as they are presented, or manipulate them using the skills you learned above. There will be **extra credit awarded for *correctly* manipulating the data**.

* Pick a numerical variable and a categorical variable from the survey and examine the relationship between them. Use numerical summaries and graphical displays. You can also manipulate the data as you like: re-level, change order of levels for better displays, subset (examine the relationship for only a sub-group), etc.

* Do the same with two categorical variables.
