...
download("https://stat.duke.edu/courses/Spring15/sta101.001/post/labs/sta101_lab5.Rmd", destfile = "sta101_lab5.Rmd")
```
</div>
...
2.  Describe the distribution of this sample? How does it compare to the 
    distribution of the population? For this question you will need to include the code for taking a sample, and plot it. Also make sure that prior to taking a sample you set a seed (`set.seed()`) in your markdown document so that each time you knit the document you work with the same random sample.
...
5000 `NA`s called `sample_means50`.  This vector will will store values 
...
5.  To make sure you understand what you've done in this loop, try running a 
    smaller version.  Initialize a vector of 100 `NA`s called 
...
=======
  samp <- sample(area, size=10)
  sample_means10[i] <- mean(samp)
  samp <- sample(area, size=100)
>>>>>>> 368f30fd2477bba6cc337a3a7f559314fb0b8d22
...
6.  When the sample size is larger, what happens to the center?  What about the spread? Make sure to include the plots in your report. You might want to increase the overall height of the figure since you're placing three plots on top of each other. To do so change the `fig.height` argument in the R chunk definition provided for this exercise. (Note: If you also wanted to customize the width of the figure, you can add a `fig.width` argument to the R chunk definition as well. The default height and width of figures is defined at the beginning of the document.)
